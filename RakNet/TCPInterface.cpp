...
bool TCPInterface::Start(unsigned short port, unsigned short maxIncomingConnections, unsigned short maxConnections, int _threadPriority, unsigned short socketFamily, const char *bindAddress)
{
	...
	{
#if defined(WINDOWS_STORE_RT)
		bool b = CreateListenSocket_WinStore8(port, maxIncomingConnections, socketFamily, bindAddress);
		if (!b)
			return false;
#else
		bool b = CreateListenSocket(port, maxIncomingConnections, socketFamily, bindAddress);
		if (!b)
			return false;
#endif
	}
